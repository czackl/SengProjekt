...
def remove_zeile(board, row):
	"""wenn eine Reihe vollständig ist, wird sie entfernt"""
	del board[row]
...
class TetrisApp(object):
	...
	def neuer_Stein(self):
		"""neue steine generieren"""
	...
	def fastdrop(self):
		"""Stein schneller als normal fallen lassen"""
		if not self.gameover and not self.paused:
			self.stone_y += 1
			if collision(self.board,
			                   self.stone,
	...
	def run(self):
		...
		while 1:
			...
			for event in pygame.event.get():
				"""die interaktionsmoeglichkeiten die der spieler hat"""
